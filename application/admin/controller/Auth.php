...
class Auth {
    ...
    protected $_config = array(
        ...
        'AUTH_GROUP'        => 'SystemAuthGroup',             //用户组数据表名
        'AUTH_GROUP_ACCESS' => 'SystemAuthGroupAccess',       //用户组明细表
        'AUTH_RULE'         => 'SystemAuthRule',              //权限规则表
//        'AUTH_USER'         => 'User'                   //用户信息表
    ...
    protected function getGroups($uid) {
        ...
        $group_access = model($this->_config['AUTH_GROUP_ACCESS'])->where(array('uid' => $uid))->find();
        if($group_access){
            $groupInfoArr = model($this->_config['AUTH_GROUP'])->where(array('id' => array('in', $group_access['groupId'])))->select();
    ...
    public function getAuthList($uid) {
        ...
        $rules = model($this->_config['AUTH_RULE'])->where(array(
