...
class Secache{
    ...
    function workat($file){
        ...
        if(!file_exists($this->_file)){
            ...
            $info['ver'] = str_replace(chr(0),'',$info['ver']);
	...
	function u_fetch($key){
        ...
        if($this->search($key,$offset)){
            ...
            if($schema_id===false){
                if($locked) $this->unlock();
                return array('status'=>-1,'msg'=>'schema_id = false');
            }
            ...
            if($return===false){
                if($locked) $this->unlock();
                return array('status'=>-1,'msg'=>'return_data = false');
            }

            if($locked){
                $this->_lru_push($schema_id,$info['offset']);
                $this->_set_schema($schema_id,'hits',$this->_get_schema($schema_id,'hits')+1);
                $unlock_flag = $this->unlock();
				return array('status'=>1,'unlock_flag'=>$unlock_flag,'data'=>$return);
            }else{
                return array('status'=>1,'data'=>$return);
            }
        }else{
            if($locked) $this->unlock();
            return array('status'=>-1,'data'=>'search no find');
        }
    }
    ...
    function store($key,$value){
		...
		echo '<hr/>store__cache__key<xmp>'.var_export($key,true).'</xmp>';
		echo '<hr/>store__cache__v<xmp>'.var_export($value,true).'</xmp>';*/
        if($this->lock(true)){
	        //debug_print_backtrace();
	        //echo '<hr/>store_key<xmp>'.var_export($key,true).'</xmp>';
	        //echo '<hr/>store_val<xmp>'.var_export($value,true).'</xmp>';
            //save data
